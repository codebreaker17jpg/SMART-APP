...
@router.post("/attendance/face", response_model=FaceAttendanceResponse)
def face_attendance(
    ...
    2. Verify the live session exists and is active.
    3. Check for duplicate attendance on the same day + subject.
    4. Insert attendance with verification_method='face'.
    """
    ...
    # 4 — Insert attendance record with face verification
    insert_resp = (
        db.table("attendance_records")
        .insert(
            {
    ...
    return FaceAttendanceResponse(
        success=True,
        message=f"Attendance marked for {student_resp.data['name']}.",
        attendance_id=insert_resp.data[0]["id"],
    )
